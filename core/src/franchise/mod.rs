...
pub fn graph(core: &Arc<Core>, series: u64) -> Result<Option<FranchiseLayout>, CoreError> {
    ...
    let built = core.store.read(|conn| {
        let matched: Option<Option<i64>> = conn
            .query_row(
                "SELECT anilist_id FROM series WHERE id = ?1",
                params![as_i64(series)],
                |r| r.get(0),
            )
            .optional()?;
        ...
        let Some(seed) = matched.map(as_u64) else {
            return Ok(None);
        };
    ...
    let Some((seed, root, layout)) = built else {
        return Ok(None);
    };
...
fn draw(
    ...
    closure: &Closure,
    current: u64,
) -> Result<FranchiseLayout, CoreError> {
    let plan = layout::plan(closure, current);
    let positions: HashMap<u64, (f64, f64)> = plan
        .positions
        .iter()
        .map(|(id, x, y)| (*id, (*x, *y)))
        .collect();
    ...
    for n in &closure.nodes {
        ...
        let Some(&(x, y)) = positions.get(&n.anilist_id) else {
            continue;
        };
...
fn title_of(n: &Node) -> String {
    n.title_romaji
        .clone()
        .or_else(|| n.title_english.clone())
        .unwrap_or_else(|| format!("AniList {}", n.anilist_id))
}
...
fn owned_by(conn: &Connection, n: &Node) -> Result<Option<u64>, CoreError> {
    ...
    let mut stmt =
        conn.prepare_cached("SELECT id FROM series WHERE anilist_id = ?1 ORDER BY id LIMIT 1")?;
    let found: Option<i64> = stmt
        .query_row(params![as_i64(n.anilist_id)], |r| r.get(0))
        .optional()?;
    Ok(found.map(as_u64))
}
...
fn list_status(conn: &Connection, anilist_id: u64) -> Result<Option<ListStatus>, CoreError> {
    let mut stmt = conn.prepare_cached(
        "SELECT status FROM tracker_entries WHERE tracker = 'anilist' AND media_id = ?1",
    )?;
    let found: Option<Option<String>> = stmt
        .query_row(params![as_i64(anilist_id)], |r| r.get(0))
        .optional()?;
...
pub fn has_graph(conn: &Connection, anilist_id: u64) -> Result<bool, CoreError> {
    ...
    let found: bool = conn.query_row(
        "SELECT EXISTS(SELECT 1 FROM relations WHERE from_id = ?1 OR to_id = ?1)",
        params![id],
        |r| r.get(0),
    )?;
    Ok(found)
...
mod tests {
    ...
    fn has_graph_answers_for_either_end_of_an_edge() {
        ...
        assert!(
            has_graph(&c, 2).unwrap(),
            "the far end of an edge has a graph too"
        );
        assert!(!has_graph(&c, 3).unwrap());
        assert!(
            !has_graph(&c, 999).unwrap(),
            "an id with no row at all has no graph"
        );
    }
}
