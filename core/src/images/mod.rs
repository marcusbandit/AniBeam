...
const REFERENCED_SQL: &str = "SELECT cover_url AS url FROM anilist_media WHERE cover_url IS NOT NULL
...
impl ImageCache {
    ...
    fn fill_gate(&self) -> MutexGuard<'_, Option<Instant>> {
        self.fill_started_at.lock().unwrap_or_else(|e| e.into_inner())
    }

    /// Whether a read that found a gap should ask for a fill.
    pub fn fill_is_due(&self) -> bool {
        self.fill_gate().is_none_or(|at| at.elapsed() >= FILL_INTERVAL)
    }
    ...
    pub async fn ensure(&self, urls: &[String]) -> Vec<(String, Result<String, CoreError>)> {
        ...
        for url in wanted {
            match known.get(&url) {
                Some(relative) if self.dir.join(relative).is_file() => {
                    done.insert(url, Ok(self.dir.join(relative).to_string_lossy().into_owned()));
                }
                Some(_) => {
        ...
        if !stale.is_empty()
            && let Err(e) = self.store.write_async(move |c| delete_rows(c, &stale)).await
        {
    ...
    pub fn path_for(&self, conn: &Connection, url: &str) -> Result<Option<String>, CoreError> {
        let relative: Option<String> = conn
            .query_row("SELECT path FROM images WHERE url = ?1", params![url], |r| r.get(0))
            .optional()?;
        Ok(relative
    ...
    pub fn bump_used(&self, urls: Vec<String>) {
        ...
        self.store.post(move |c| {
            let sql = format!("UPDATE images SET used_at = ? WHERE url IN ({}) AND used_at < ?", placeholders(urls.len()));
    ...
    pub fn storage(&self, conn: &Connection) -> Result<(u64, u64), CoreError> {
        let (count, bytes): (i64, i64) =
            conn.query_row("SELECT count(*), COALESCE(sum(bytes), 0) FROM images", [], |r| Ok((r.get(0)?, r.get(1)?)))?;
    ...
    fn sweep_to(&self, conn: &mut Connection, now: i64, cap: u64) -> Result<SweepReport, CoreError> {
        ...
        let keep: HashSet<String>;
        {
            ...
            if rows.len() as u64 > cap {
                let mut candidates: Vec<usize> = (0..rows.len()).filter(|i| !pinned.contains(&rows[*i].url)).collect();
                candidates.sort_by_key(|i| (u8::from(referenced.contains(&rows[*i].url)), rows[*i].used_at));
                let over = (rows.len() as u64 - cap) as usize;
                let evicted: HashSet<String> = candidates.into_iter().take(over).map(|i| rows[i].url.clone()).collect();
    ...
    fn delete_files_outside(&self, keep: &HashSet<String>) -> u64 {
        let mut removed = 0;
        for entry in walkdir::WalkDir::new(&self.dir).into_iter().filter_map(Result::ok) {
            if !entry.file_type().is_file() {
                continue;
            }
            let Ok(relative) = entry.path().strip_prefix(&self.dir) else { continue };
...
fn all_rows(conn: &Connection) -> Result<Vec<CachedRow>, CoreError> {
    ...
    let rows = stmt.query_map([], |r| Ok(CachedRow { url: r.get(0)?, path: r.get(1)?, used_at: r.get(2)? }))?;
...
fn rows_for(conn: &Connection, urls: &[String]) -> Result<HashMap<String, String>, CoreError> {
    ...
    for chunk in urls.chunks(500) {
        let sql = format!("SELECT url, path FROM images WHERE url IN ({})", placeholders(chunk.len()));
        let mut stmt = conn.prepare(&sql)?;
        let rows = stmt.query_map(params_from_iter(chunk.iter()), |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))?;
        for row in rows {
            let (url, path) = row?;
            out.insert(url, path);
        }
...
fn delete_rows(conn: &Connection, urls: &[String]) -> Result<(), CoreError> {
    for chunk in urls.chunks(500) {
        let sql = format!("DELETE FROM images WHERE url IN ({})", placeholders(chunk.len()));
...
fn series_for_urls(conn: &Connection, urls: &[String]) -> Result<Vec<u64>, CoreError> {
    ...
    let mut stmt = conn.prepare(&sql)?;
    let rows = stmt.query_map(params_from_iter(urls.iter().chain(urls.iter())), |r| r.get::<_, i64>(0))?;
    Ok(rows.collect::<Result<Vec<_>, _>>()?.into_iter().map(|id| id as u64).collect())
}

fn matched_series(conn: &Connection) -> Result<Vec<u64>, CoreError> {
    let mut stmt = conn.prepare("SELECT id FROM series WHERE anilist_id IS NOT NULL ORDER BY id")?;
...
/// wears the real name.
async fn fetch_one(http: &dyn Http, store: &Store, dir: &Path, url: &str) -> Result<String, CoreError> {
    let request = HttpRequest { method: Method::Get, url: url.to_string(), headers: Vec::new(), body: None };
    let response = http.send(request).await.map_err(|e| cdn_error(None, e.message))?;
    if !response.is_success() {
        return Err(cdn_error(Some(u32::from(response.status)), format!("image fetch failed: {}", response.text())));
    }
    let relative = relative_path(url, extension(url, response.header("content-type")));
    let path = dir.join(&relative);
    let parent = path.parent().ok_or_else(|| CoreError::internal("an image path has no parent"))?;
...
pub fn start_fill(core: &Arc<Core>) -> u64 {
    ...
    let id = core.jobs.clone().start(JobKind::FillImages, move |ctx| async move {
        ...
        for (round, batch) in urls.chunks(BATCH).enumerate() {
            ...
            if !arrived.is_empty() {
                touched.extend(owner.store.write_async(move |c| series_for_urls(c, &arrived)).await?);
            }
        }
        ...
        if !touched.is_empty() {
            ...
            let cards = owner.store.write_async(move |c| cards::cards_for(c, &dir, &ids)).await?;
            ctx.emit(Level::Debug, format!("fetched {fetched} images"), EventBody::SeriesChanged { series: cards });
        }
        Ok(Finished {
            level: Level::Info,
...
pub fn start_clear(core: &Arc<Core>) -> u64 {
    let owner = core.clone();
    core.jobs.clone().start(JobKind::ClearImages, move |ctx| async move {
        let removed = owner
            .store
            .write_async(|c| {
                let count: i64 = c.query_row("SELECT count(*) FROM images", [], |r| r.get(0))?;
        ...
        if !ids.is_empty() {
            ...
            let cards = owner.store.write_async(move |c| cards::cards_for(c, &images_dir, &ids)).await?;
            ctx.emit(Level::Debug, format!("{} series lost their images", cards.len()), EventBody::SeriesChanged { series: cards });
        }
        Ok(Finished {
            level: Level::Info,
...
mod tests {
    ...
    fn the_key_is_the_urls_sha256_and_the_path_shards_on_its_first_two_hex() {
        ...
        assert_eq!(key("https://img/xl.jpg"), k, "the same url always hashes the same");
        assert_ne!(key("https://img/other.jpg"), k);
        assert_eq!(relative_path("https://img/xl.jpg", "jpg"), format!("{}/{k}.jpg", &k[..2]));
    }
    ...
    fn the_extension_comes_from_the_url_then_the_content_type_then_jpg() {
        ...
        assert_eq!(extension("https://img/nodot", Some("image/webp; charset=binary")), "webp");
        assert_eq!(extension("https://img/nodot", Some("application/octet-stream")), "jpg");
        assert_eq!(extension("https://img/nodot", None), "jpg");
        assert_eq!(extension("https://cdn.example.com/no-extension-here", None), "jpg");
    }
    ...
    fn bench() -> Bench {
        ...
        Bench { _dir: dir, store, cache, now: 1_700_000_000 }
    }
    ...
    fn the_sweep_drops_lost_rows_stray_files_and_old_orphans_then_caps_what_is_left() {
        ...
        assert_eq!(b.urls(), vec!["pinned-cover".to_string(), "portrait".to_string()]);
        assert_eq!(b.files(), 2);
    }
    ...
    fn bump_used_only_moves_an_image_a_day_stale_and_path_for_ignores_a_lost_file() {
        ...
        b.cache.bump_used(vec!["fresh".into(), "stale".into(), "lost".into()]);
        ...
        let used: Vec<(String, i64)> = b
            ...
            .unwrap();
        let at = |url: &str| used.iter().find(|(u, _)| u == url).map(|(_, at)| *at).unwrap();
        assert_eq!(at("fresh"), now, "a bump inside the day is not worth a write");
    ...
    async fn ensure_returns_the_path_once_and_never_fetches_a_cached_url_twice() {
        ...
        let urls = vec!["https://img/one.jpg".to_string(), "https://img/two.jpg".to_string()];
        let out = cache.ensure(&urls).await;
        assert_eq!(out.len(), 2);
        let one = out[0].1.clone().unwrap();
        assert!(std::path::Path::new(&one).is_file());
        assert!(matches!(out[1].1, Err(CoreError::Provider { status: Some(500), .. })));
