...
use super::{has_extension, BRACKETS, SPACES};
...
static SEASON_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![Regex::new(r"(?i)Season\s*([0-9]+)").unwrap(), Regex::new(r"(?i)\bS([0-9]+)\b").unwrap()]
});
static PART_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![Regex::new(r"(?i)Part\s*([0-9]+)").unwrap(), Regex::new(r"(?i)\bP([0-9]+)\b").unwrap()]
});
static EPISODE_RANGE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)\s+-\s+[0-9]+\s*[~\x{2013}-]\s*[0-9]+(\s+END)?\s*$").unwrap());
...
fn find_case_insensitive(haystack: &str, needle: &str) -> Option<std::ops::Range<usize>> {
    ...
    for start in 0..=(total_chars - needle_chars) {
        ...
        let end_byte = positions.get(start + needle_chars).copied().unwrap_or(haystack.len());
...
impl WalkState {
    fn new(root: &Path) -> Self {
        ...
        Self { listings: HashMap::new(), visited }
    }
    ...
    fn list(&mut self, dir: &Path) -> Rc<Listing> {
        ...
        let raw = list(dir);
        let dirs: Vec<PathBuf> = raw.dirs.into_iter().filter(|d| self.visited.insert(canonical_path(d))).collect();
        let l = Rc::new(Listing { dirs, videos: raw.videos, subtitles: raw.subtitles });
...
fn classify_folder(state: &mut WalkState, loose_videos: usize, dirs: &[PathBuf]) -> Shape {
    let video_bearing: Vec<PathBuf> = dirs.iter().filter(|d| has_videos_shallow(state, d)).cloned().collect();
...
fn film(state: &mut WalkState, p: &Path, use_folder_title: bool) -> ScannedSeries {
    ...
    let subtitles: &[String] = listing.as_deref().map(|l| l.subtitles.as_slice()).unwrap_or(&[]);
...
/// ancestor.
fn collect_videos_in_subtree(state: &mut WalkState, root: &Path, inherited: Option<u32>) -> Vec<ScannedFile> {
    fn visit(state: &mut WalkState, dir: &Path, parent_season: Option<u32>, out: &mut Vec<ScannedFile>, seen: &mut HashSet<String>) {
...
fn collect(
    ...
    match classify_folder(state, l.videos.len(), &l.dirs) {
        Shape::Passthrough(sub) => {
            ...
            collect(state, &sub, results, false, false, None, Some(Context { name: canonical, season_hint: hint }));
        }
        Shape::Wrapper(subs) => {
            for v in &l.videos {
                results.push(film(state, v, false));
            ...
            for sub in subs {
                ...
                collect(state, &sub, results, false, false, Some(Context { name, season_hint }), None);
            }
        }
        Shape::Series => {
...
mod tests {
    ...
    fn helpers() {
        ...
        assert_eq!(
            clean_folder_title("[Erai-raws] Karakai Jouzu no Takagi-san 2 - 01 ~ 12 [1080p][Multiple Subtitle]"),
            "Karakai Jouzu no Takagi-san 2"
        );
        assert_eq!(clean_folder_title("Show - 01-12 END"), "Show");
        assert_eq!(
            derive_subfolder_series_name("[Erai-raws] Karakai Jouzu no Takagi-san 2 - 01 ~ 12 [1080p]", "Karakai Jouzu no Takagi-san"),
        ...
        assert_eq!(clean_movie_title("Perfect.Blue.1997.1080p.BluRay.mkv"), "Perfect Blue 1080p BluRay");
    ...
    fn derive_subfolder_series_name_survives_unicode_case_folding() {
        ...
        assert_eq!(derive_subfolder_series_name("KShowあ", "Show"), ("Show あ".to_string(), None));
    }
    ...
    fn a_transparent_release_group_wrapper_stays_one_series() {
        let tmp = tempfile::tempdir().unwrap();
        for ep in 1..=3 {
            touch(&tmp.path().join("My Show").join("[Some-Group]").join(format!("My Show - {ep:02}.mkv")));
        }
        let results = scan_source(tmp.path()).unwrap();
    ...
    fn a_flat_series_is_one_series() {
        let tmp = tempfile::tempdir().unwrap();
        for ep in 1..=4 {
            touch(&tmp.path().join("Plain Show").join(format!("Plain Show - {ep:02}.mkv")));
        }
        let results = scan_source(tmp.path()).unwrap();
    ...
    fn one_subdir_plus_one_loose_video_is_a_wrapper() {
        ...
        for ep in 1..=3 {
            touch(&show.join("[Group]").join(format!("Edge Show - {ep:02}.mkv")));
        }
    ...
    fn season_folders_behind_a_release_folder_split_by_season() {
        ...
        for s in 1..=3 {
            for ep in 1..=12 {
                touch(&release.join(format!("[Judas] Kaminomi - S{s}")).join(format!("[Judas] Kami nomi zo Shiru Sekai - S0{s}E{ep:02}.mkv")));
            }
        }
        for f in [
    ...
    fn plain_season_subfolders_take_the_shows_name() {
        ...
        for (dir, n) in [("Season 1", 3), ("Season 2", 3), ("Specials", 2)] {
            for ep in 1..=n {
                touch(&tmp.path().join("Some Show").join(dir).join(format!("{dir} - {ep:02}.mkv")));
            }
        }
    ...
    fn movies_folders_films_and_sidecars() {
        ...
        assert_eq!(
            ...
            vec![Sidecar { path: tmp.path().join("Movies/Perfect Blue (1997).en.srt").to_string_lossy().into_owned(), language: Some("en".into()), title: None }]
        );
