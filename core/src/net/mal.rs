...
mod tests {
    ...
    fn client(http: Arc<FakeHttp>) -> MalClient {
        MalClient::new(ProviderClient::new(
            Upstream::Mal,
            http,
    ...
    async fn a_get_carries_the_bearer_token() {
        ...
        let r = client(http.clone())
            .get("https://api.myanimelist.net/v2/users/@me", "t0ken")
            .await
            .unwrap();
        ...
        assert!(
            req.headers
                .contains(&("Authorization".to_string(), "Bearer t0ken".to_string()))
        );
    }
    ...
    async fn a_patch_is_a_form_with_the_token_and_a_post_is_a_form_without_one() {
        ...
        .await
        .unwrap();
        c.post_form("https://myanimelist.net/v1/oauth2/token", form.clone())
            .await
            .unwrap();
