...
static ED_PATTERN: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)^(outro|ending|closing|credits|end\s*credits|ed(?:\s*[0-9]+)?)\b").unwrap());
...
pub fn from_chapters(chapters: &[Chapter], duration: f64) -> Vec<SkipWindow> {
    ...
    for (i, chapter) in chapters.iter().enumerate() {
        ...
        let (start, end) = (chapter.start, chapters.get(i + 1).map_or(duration, |next| next.start));
        ...
        if intro.is_none() && OP_PATTERN.is_match(title) {
            intro = Some(SkipWindow { kind: SkipKind::Intro, start, end, source: SkipSource::Chapters });
        } else if outro.is_none() && ED_PATTERN.is_match(title) {
            outro = Some(SkipWindow { kind: SkipKind::Outro, start, end, source: SkipSource::Chapters });
        }
    }
...
pub fn cached(conn: &Connection, series: u64, key: &str, now: i64) -> Result<Option<Vec<SkipWindow>>, CoreError> {
    let row: Option<(String, String, i64)> = conn
        ...
        .optional()?;
    let Some((raw, source, fetched_at)) = row else { return Ok(None) };
...
async fn write_cache(core: &Core, s: &Session, windows: &[SkipWindow], source: &str) -> Result<(), CoreError> {
    let json = serde_json::to_string(windows)?;
    let (series, key, source, now) = (s.series, s.episode_key.clone(), source.to_string(), time::now_secs());
    core.store
        .write_async(move |c| {
            c.execute(
...
pub fn start(core: &Core, session: u64, chapters: Vec<Chapter>, duration: f64) -> Result<u64, CoreError> {
    let s = core.sessions.get(session).ok_or(CoreError::NotFound { what: Entity::Session, id: session })?;
    if !duration.is_finite() || duration <= 0.0 {
        return Err(CoreError::invalid("duration", "a duration is a positive number of seconds"));
    }
    session::report_chapters(core, session, duration, None);
    let owner = owner(core)?;
    Ok(owner.jobs.clone().start(JobKind::SkipWindows, move |ctx| async move {
        ...
        Ok(Finished {
            ...
            body: EventBody::SkipWindowsReady { session: s.id, windows },
        })
    }))
}
...
async fn resolve(
    ...
    if let Some(windows) = core.store.write_async(move |c| cached(c, series, &key, now)).await? {
        return Ok(windows);
    }
    ...
    let (Some(mal_id), Some(number)) = (s.mal_id, s.number) else { return Ok(Vec::new()) };
    ...
    match core.aniskip.skip_times(mal_id, episode, duration.round() as u64).await {
        Ok(Some(windows)) => {
        ...
        Err(e) => {
            ctx.emit(Level::Warn, format!("AniSkip did not answer for episode {episode}: {}", message_of(&e)), EventBody::Notice);
            Ok(Vec::new())
        }
    }
}
...
mod tests {
    ...
    fn intro(start: f64, end: f64) -> SkipWindow {
        SkipWindow { kind: SkipKind::Intro, start, end, source: SkipSource::Chapters }
    }

    fn outro(start: f64, end: f64) -> SkipWindow {
        SkipWindow { kind: SkipKind::Outro, start, end, source: SkipSource::Chapters }
    }
    ...
    fn a_chapters_span_runs_to_the_next_one() {
        let chapters = vec![chapter("Opening", 0.0), chapter("Part A", 90.0), chapter("Ending", 1300.0)];
        assert_eq!(from_chapters(&chapters, 1400.0), vec![intro(0.0, 90.0), outro(1300.0, 1400.0)]);
    }
    ...
    fn numbers_and_longer_words_never_match() {
        for title in ["Episode 1", "Chapter 1", "Operations", "Editorial", "", "   "] {
            let chapters = vec![chapter(title, 0.0), chapter("Part A", 90.0)];
            assert!(from_chapters(&chapters, 1400.0).is_empty(), "{title} matched");
        }
        // A numbered opening or ending is still one.
        let chapters = vec![chapter("OP 2", 0.0), chapter("Part A", 90.0), chapter("ED2", 1300.0)];
        assert_eq!(from_chapters(&chapters, 1400.0), vec![intro(0.0, 90.0), outro(1300.0, 1400.0)]);
    }
    ...
    fn a_chapter_longer_than_five_minutes_is_ignored() {
        ...
        assert_eq!(from_chapters(&[chapter("Opening", 0.0)], 300.0), vec![intro(0.0, 300.0)]);
    ...
    fn the_first_of_each_kind_wins_and_a_bad_span_is_skipped() {
        ...
        assert_eq!(from_chapters(&chapters, 1400.0), vec![intro(0.0, 90.0), outro(1300.0, 1350.0)]);

        // A chapter that starts where the one after it does, or after it,
        // has no span to skip.
        assert!(from_chapters(&[chapter("Opening", 90.0), chapter("Part A", 90.0)], 1400.0).is_empty());
        assert!(from_chapters(&[chapter("Opening", f64::NAN), chapter("Part A", 90.0)], 1400.0).is_empty());
