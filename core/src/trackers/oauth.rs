...
/// that is not a complete escape stands for itself rather than failing.
/// Bytes that do not make UTF-8 become the replacement character, so a
/// mangled query is still a string rather than a panic.
...
mod tests {
    ...
    /// A truncated or malformed escape stands for itself rather than
    /// ending the flow: nothing here panics on what a browser sends.
    #[test]
    fn a_broken_escape_survives_decoding() {
