...
fn attempted_at(core: &Core, series: u64) -> Option<i64> {
    core.store()
        .read(|c| Ok(c.query_row("SELECT attempted_at FROM series WHERE id = ?1", params![series as i64], |r| r.get(0))?))
        .unwrap()
}
...
fn auto_match_matches_once_at_the_gate_and_never_rehammers() {
    ...
    assert!(c.events().iter().any(|e| e.message == "no match for Zzz Unknown (threshold 0.5)"), "{:#?}", c.events());
    assert!(c.events().iter().any(|e| e.message == "match (AniList 1.00): Sousou no Frieren -> Sousou no Frieren"), "{:#?}", c.events());

    let cards = list_cards(&core);
    let matched = cards.iter().find(|s| s.id == hit).unwrap();
    assert_eq!(
        matched.match_info.as_ref().map(|m| (m.provider, m.anilist_id, m.mal_id, m.confirmed)),
    ...
    assert!(cleared.iter().find(|s| s.id == hit).unwrap().match_info.is_none());
    assert!(attempted_at(&core, hit).is_some(), "clearing a match must not clear the attempt");
    let job = started(&core, Call::AutoMatch);
    let done = common::wait_job(&c, job);
    assert!(matches!(done.body, EventBody::AutoMatchFinished { matched: 0, .. }), "{done:?}");
    assert_eq!(http.requests().len(), before);

    assert!(matches!(core.call(Call::ClearMatch { series: 9999 }), Err(CoreError::NotFound { what: Entity::Series, id: 9999 })));
    core.shutdown();
}
...
fn an_exhausted_rate_limit_fails_the_job_and_stamps_nothing() {
    ...
    // 32 second schedule.
    for _ in 0..7 {
        http.push_with_headers(429, "rate limited", vec![("Retry-After".to_string(), "0".to_string())]);
    }

    let job = started(&core, Call::AutoMatch);
    let done = common::wait_job(&c, job);
    assert!(matches!(done.body, EventBody::JobFailed { error: CoreError::Provider { status: Some(429), .. } }), "{done:?}");
    assert!(attempted_at(&core, series).is_none(), "a rate limit must not count as an attempt");
    core.shutdown();
}
