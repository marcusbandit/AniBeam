...
fn list_series_tabs_cards_detail_and_metadata() {
    ...
    for n in 1..=8 {
        fixtures::insert_file(
            &core,
            frieren,
            &format!("/lib/Sousou no Frieren/{n:02}.mkv"),
            n as f64,
            None,
            "episode",
            now - 100 * n,
    ...
    fixtures::insert_file(
        &core,
        frieren,
        "/lib/Sousou no Frieren/NCOP1.mkv",
        1.0,
        None,
        "extra",
        now,
    ...
    let film = fixtures::insert_series(
        ...
        src,
        SeriesKind::Movie,
        "/lib/Movies/Perfect Blue (1997).mkv",
        "Perfect Blue",
    );
    fixtures::insert_file(
        &core,
    ...
    assert_eq!(
        ...
        UNIX_EPOCH + Duration::from_secs((now - 86400) as u64)
    );
        ...
        .unwrap()
    {
        Reply::Metadata { rows, counts } => {
            ...
            assert_eq!(
                (r.have, r.expected, r.extra_on_disk, r.provider),
                (8, Some(28), 0, Some(Provider::Anilist))
            );
            assert_eq!(r.alt_title.as_deref(), Some("Sousou no Frieren"));
            assert!(
                rows.iter()
                    .any(|r| r.series.id == missing && r.series.missing)
            );
        }
        other => panic!("{other:?}"),
    }
    ...
    match core
        .call(Call::ListAiring {
            offset: 0,
            limit: 10,
        })
        .unwrap()
    {
        Reply::Series { series } => assert_eq!(
    ...
    assert!(
        matches!(core.call(Call::Lookup { path: "/lib/Sousou no Frieren".into() }).unwrap(), Reply::Lookup { series: Some(s), file: None } if s == frieren)
    );
    assert!(matches!(
        core.call(Call::Lookup {
            path: "/lib/Sousou no Frieren/03.mkv".into()
        })
        .unwrap(),
        Reply::Lookup {
            series: Some(_),
    ...
    assert!(matches!(
        core.call(Call::GetSeries { series: 9999 }),
