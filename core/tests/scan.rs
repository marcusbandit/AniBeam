...
fn add_source_scans_reconciles_marks_missing_and_forgets() {
    ...
    let cards = match core.call(Call::ListSeries { tab: Tab::All, query: String::new(), sort: Sort::Alpha, direction: Direction::Asc, reveal_hidden: false }).unwrap() {
...
fn a_reclassified_file_changes_series_without_colliding_on_its_path() {
    ...
    let lib = dir.path().join("lib");
    for n in 1..=3 {
        touch(&lib.join("Show A").join("Season 1").join(format!("Show A - {n:02}.mkv")));
    }
    let source = match core.call(Call::AddSource { path: lib.to_string_lossy().into_owned() }).unwrap() {
    ...
    let scan = common::wait_for(&c, |e| matches!(e.body, EventBody::JobStarted { kind: JobKind::Scan }), std::time::Duration::from_secs(5)).job.unwrap().id;
    let done = common::wait_job(&c, scan);
    assert!(matches!(done.body, EventBody::ScanFinished { added: 1, .. }), "{done:?}");

    for n in 1..=2 {
        touch(&lib.join("Show A").join("Season 2").join(format!("Show A - S02E{n:02}.mkv")));
    }
    let job = started(core.call(Call::Scan { source: Some(source) }).unwrap());
    let done = common::wait_job(&c, job);
    assert!(matches!(done.body, EventBody::ScanFinished { added: 2, changed: 0, removed: 1, .. }), "{done:?}");
    ...
    assert!(cards.iter().any(|s| s.path == lib.join("Show A").join("Season 1").to_string_lossy()));
...
fn an_overlapping_source_is_refused_at_the_door() {
    ...
    match core.call(Call::AddSource { path: format!("{}/", lib.to_string_lossy()) }).unwrap() {
    ...
    assert_eq!(refusal(lib.to_string_lossy().into_owned()), "already a source");
    assert_eq!(refusal(format!("{}/", lib.to_string_lossy())), "already a source");
    assert_eq!(refusal(lib.join("Show A").to_string_lossy().into_owned()), "nested inside an existing source");
    assert_eq!(refusal(dir.path().to_string_lossy().into_owned()), "nested inside an existing source");
    ...
    assert!(matches!(core.call(Call::AddSource { path: other.to_string_lossy().into_owned() }).unwrap(), Reply::Source { .. }));

    let job = started(core.call(Call::Scan { source: None }).unwrap());
    let done = common::wait_job(&c, job);
    assert!(matches!(done.body, EventBody::ScanFinished { .. }), "{done:?}");
}
