...
fn graphql_variables(http: &FakeHttp, operation: &str) -> Vec<serde_json::Value> {
    http.requests()
        ...
        .filter_map(|r| match r.body {
            Some(Body::Json(v)) if v["query"].as_str().is_some_and(|q| q.contains(operation)) => Some(v["variables"].clone()),
            _ => None,
        })
        .collect()
}
...
fn outcome(outcomes: &[TrackerOutcome], t: Tracker) -> TrackerOutcome {
    outcomes.iter().find(|o| o.tracker == t).unwrap_or_else(|| panic!("no {t:?} outcome in {outcomes:?}")).clone()
}
...
fn a_mark_writes_to_both_trackers_and_the_card_follows() {
    ...
    http.push_for("anilist", 200, r#"{"data":{"MediaList":{"progress":11,"status":"CURRENT"}}}"#);
    http.push_for("anilist", 200, r#"{"data":{"SaveMediaListEntry":{"id":9,"progress":12,"status":"COMPLETED"}}}"#);
    http.push_for("myanimelist", 404, r#"{"error":"not_found"}"#);
    http.push_for("myanimelist", 200, "{}");

    let job = started(core.call(Call::MarkEpisode { series, episode: 12.5 }).unwrap());
    let done = common::wait_job(&c, job);
    let (episode, outcomes) = match done.body {
        EventBody::Marked { episode, outcomes, .. } => (episode, outcomes),
        other => panic!("{other:?}"),
    };
    ...
    assert_eq!(outcome(&outcomes, Tracker::Anilist), TrackerOutcome { tracker: Tracker::Anilist, ok: true, progress: Some(12), reason: None, message: None });
    assert_eq!(outcome(&outcomes, Tracker::Mal), TrackerOutcome { tracker: Tracker::Mal, ok: true, progress: Some(12), reason: None, message: None });
    ...
    assert_eq!(graphql_variables(&http, "MediaList(userId:"), vec![serde_json::json!({ "userId": 42, "mediaId": 1 })]);
    let vars = graphql_variables(&http, "SaveMediaListEntry");
    assert_eq!(vars.len(), 1);
    assert_eq!(vars[0], serde_json::json!({ "mediaId": 1, "progress": 12, "status": "COMPLETED" }));
    let forms = mal_forms(&http);
    assert_eq!(forms.len(), 1);
    assert!(forms[0].contains(&("num_watched_episodes".to_string(), "12".to_string())), "{:?}", forms[0]);
    assert!(forms[0].contains(&("status".to_string(), "completed".to_string())), "{:?}", forms[0]);
    ...
    assert!(
        c.events().iter().any(|e| e.message == "anilist 11 -> 12 (mediaId 1)"),
        "{:#?}",
...
fn a_mark_the_list_already_covers_is_refused_at_once() {
    ...
    let err = core.call(Call::MarkEpisode { series, episode: 12.0 }).err().unwrap();
    assert_eq!(err, CoreError::Refused { reason: Refusal::NotNewer });
...
fn a_hidden_series_is_refused() {
    ...
    let series = library(&core);
    fixtures::connect_tracker(&core, Tracker::Anilist, 42, "atok");
    core.call(Call::SetHidden { series, hidden: true }).unwrap();

    for call in [Call::MarkEpisode { series, episode: 3.0 }, Call::SetProgress { series, progress: 3 }, Call::SetScore { series, score: Some(8.0) }] {
        assert_eq!(core.call(call).err().unwrap(), CoreError::Refused { reason: Refusal::Hidden });
    }
    assert!(http.requests().is_empty());
}
...
fn an_unmatched_series_is_refused() {
    ...
    assert_eq!(core.call(Call::MarkEpisode { series, episode: 1.0 }).err().unwrap(), CoreError::Refused { reason: Refusal::NoMatch });
    assert_eq!(core.call(Call::SetProgress { series, progress: 1 }).err().unwrap(), CoreError::Refused { reason: Refusal::NoMatch });
    assert_eq!(core.call(Call::SetScore { series, score: None }).err().unwrap(), CoreError::Refused { reason: Refusal::NoMatch });
    assert!(http.requests().is_empty());
}
...
fn no_connected_tracker_names_the_main_one() {
    ...
    assert_eq!(core.call(Call::MarkEpisode { series, episode: 1.0 }).err().unwrap(), CoreError::NotConnected { tracker: Tracker::Anilist });
    core.call(Call::SetMainTracker { tracker: Tracker::Mal }).unwrap();
    assert_eq!(core.call(Call::SetProgress { series, progress: 1 }).err().unwrap(), CoreError::NotConnected { tracker: Tracker::Mal });
    assert!(http.requests().is_empty());
}
...
fn a_missing_series_is_not_found() {
    ...
    let expected = CoreError::NotFound { what: Entity::Series, id: series };
    assert_eq!(core.call(Call::MarkEpisode { series, episode: 1.0 }).err().unwrap(), expected);
    assert_eq!(core.call(Call::SetProgress { series, progress: 1 }).err().unwrap(), expected);
    assert_eq!(core.call(Call::SetScore { series, score: Some(8.0) }).err().unwrap(), expected);
    // A series that never existed answers the same way.
    assert_eq!(core.call(Call::MarkEpisode { series: 9999, episode: 1.0 }).err().unwrap(), CoreError::NotFound { what: Entity::Series, id: 9999 });
    assert!(http.requests().is_empty());
}
...
fn an_impossible_episode_or_score_is_invalid() {
    ...
    for episode in [0.0, 0.5, -1.0, f64::NAN] {
        let err = core.call(Call::MarkEpisode { series, episode }).err().unwrap();
        assert!(matches!(&err, CoreError::Invalid { field, .. } if field == "episode"), "{episode} gave {err:?}");
    }
    for score in [-0.1, 10.5, f64::INFINITY] {
        let err = core.call(Call::SetScore { series, score: Some(score) }).err().unwrap();
        assert!(matches!(&err, CoreError::Invalid { field, .. } if field == "score"), "{score} gave {err:?}");
    }
    assert!(http.requests().is_empty());
}
...
fn setting_progress_to_nought_plans_the_series() {
    ...
    http.push_for("anilist", 200, r#"{"data":{"MediaList":{"progress":5,"status":"CURRENT"}}}"#);
    http.push_for("anilist", 200, r#"{"data":{"SaveMediaListEntry":{"id":9,"progress":0,"status":"PLANNING"}}}"#);

    let job = started(core.call(Call::SetProgress { series, progress: 0 }).unwrap());
    let done = common::wait_job(&c, job);
    let (progress, outcomes) = match done.body {
        EventBody::ProgressSet { progress, outcomes, .. } => (progress, outcomes),
        other => panic!("{other:?}"),
    };
    ...
    let vars = graphql_variables(&http, "SaveMediaListEntry");
    assert_eq!(vars, vec![serde_json::json!({ "mediaId": 1, "progress": 0, "status": "PLANNING" })]);
    assert!(
        c.events().iter().any(|e| e.message == "anilist set 5 -> 0 (mediaId 1)"),
        "{:#?}",
        c.events().iter().map(|e| e.message.clone()).collect::<Vec<_>>()
    );
...
fn a_score_goes_to_both_trackers_and_completes_a_finished_list() {
    ...
    http.push_for("anilist", 200, r#"{"data":{"MediaList":{"progress":12,"status":"CURRENT"}}}"#);
    http.push_for("anilist", 200, r#"{"data":{"SaveMediaListEntry":{"id":9,"score":8.7,"status":"COMPLETED"}}}"#);
    http.push_for("myanimelist", 200, r#"{"my_list_status":{"num_episodes_watched":12}}"#);
    http.push_for("myanimelist", 200, "{}");

    let job = started(core.call(Call::SetScore { series, score: Some(8.7) }).unwrap());
    let done = common::wait_job(&c, job);
    let (score, outcomes) = match done.body {
        EventBody::Scored { score, outcomes, .. } => (score, outcomes),
        other => panic!("{other:?}"),
    };
    ...
    assert_eq!(graphql_variables(&http, "SaveMediaListEntry"), vec![serde_json::json!({ "mediaId": 1, "scoreRaw": 87, "status": "COMPLETED" })]);
    let forms = mal_forms(&http);
    assert_eq!(forms.len(), 1);
    // 8.7 rounds the way MAL's own UI rounds a typed decimal.
    assert!(forms[0].contains(&("score".to_string(), "9".to_string())), "{:?}", forms[0]);
    assert!(forms[0].contains(&("status".to_string(), "completed".to_string())), "{:?}", forms[0]);
    ...
    http.push_for("anilist", 200, r#"{"data":{"MediaList":{"progress":3,"status":"CURRENT"}}}"#);
    http.push_for("anilist", 200, r#"{"data":{"SaveMediaListEntry":{"id":9,"score":5}}}"#);
    http.push_for("myanimelist", 200, r#"{"my_list_status":{"num_episodes_watched":3}}"#);
    http.push_for("myanimelist", 200, "{}");
    let job = started(core.call(Call::SetScore { series, score: Some(5.0) }).unwrap());
    ...
    assert!(!forms[1].iter().any(|(k, _)| k == "status"), "{:?}", forms[1]);
}
...
fn an_exhausted_rate_limit_is_one_sanitised_outcome() {
    ...
    for _ in 0..7 {
        http.push_with_headers(429, "slow down", vec![("Retry-After".to_string(), "0".to_string())]);
    }

    let job = started(core.call(Call::MarkEpisode { series, episode: 5.0 }).unwrap());
    let done = common::wait_job(&c, job);
    ...
    assert!(!anilist.ok);
    assert_eq!(anilist.message.as_deref(), Some("AniList rate limited, try again in a minute."));
