...
fn stage(dir: &Path) -> String {
    ...
    fs::write(lib.join("Sousou no Frieren").join("Sousou no Frieren - 01.mkv"), b"x").unwrap();
...
fn metadata(core: &Core) -> Vec<MetadataRow> {
    match core.call(Call::ListMetadata { filter: MetadataFilter::All, query: String::new(), reveal_hidden: true }).unwrap() {
...
fn views(core: &Core) -> Vec<(String, String)> {
    rows(core, "SELECT s.path, v.episode_key FROM views v JOIN series s ON s.id = v.series_id ORDER BY s.path")
}

fn completed(core: &Core) -> Vec<(String, String)> {
    rows(core, "SELECT s.path, c.episode_key FROM completed c JOIN series s ON s.id = c.series_id ORDER BY c.episode_key")
}

fn resume_points(core: &Core) -> Vec<(String, String)> {
    rows(core, "SELECT s.path, r.episode_key FROM resume_points r JOIN series s ON s.id = r.series_id ORDER BY r.episode_key")
}
...
fn the_spec_document_imports_and_a_second_import_changes_nothing() {
    ...
    // call of its own.
    assert!(c.bodies().iter().any(|b| matches!(b, EventBody::SourceChanged { source } if source.path.ends_with("anime"))));
    assert!(c.bodies().iter().any(|b| matches!(b, EventBody::SeriesChanged { series } if series.len() == 3)));
    assert!(c.bodies().iter().any(|b| matches!(b, EventBody::TrackersChanged { .. })));
    assert!(c.bodies().iter().any(|b| matches!(b, EventBody::PreferencesChanged { .. })));

    let rows = metadata(&core);
    assert_eq!(rows.len(), 3);
    let frieren = rows.iter().find(|r| r.series.path.ends_with("Sousou no Frieren")).unwrap();
    assert_eq!(
        frieren.series.match_info,
        Some(MatchInfo { provider: Provider::Anilist, anilist_id: Some(154587), mal_id: Some(52991), tmdb_id: None, tmdb_kind: None, confirmed: true })
    );
    assert!(!frieren.series.missing && !frieren.series.hidden);

    let film = rows.iter().find(|r| r.series.kind == SeriesKind::Movie).unwrap();
    assert!(film.series.path.ends_with("Perfect Blue (1997).mkv"));
    assert_eq!(
        film.series.match_info,
        Some(MatchInfo { provider: Provider::Tmdb, anilist_id: None, mal_id: None, tmdb_id: Some(10494), tmdb_kind: Some(TmdbKind::Movie), confirmed: true })
    );

    // A path that was never there is the same missing state a scan leaves,
    // so the same scan attaches its files later.
    let unmatched = rows.iter().find(|r| r.series.path.ends_with("Some Unmatched Folder")).unwrap();
    assert!(unmatched.series.hidden && unmatched.series.missing && unmatched.series.match_info.is_none());
...
fn a_private_export_round_trips_into_a_fresh_core() {
    ...
    let frieren = metadata(&core).into_iter().find(|r| r.series.path.ends_with("Sousou no Frieren")).unwrap().series.id;
    let audio = TrackRef { kind: TrackKind::Embedded, language: Some("ja".to_string()), title: None };
    core.call(Call::SetTrackChoice { series: frieren, audio: Some(audio.clone()), subtitle: Some(SubtitleChoice::Off) }).unwrap();
    ...
    core.call(Call::SetPreferences { preferences: preferences.clone() }).unwrap();
    core.call(Call::SetAutoSkip { intro: true, outro: false }).unwrap();

    let out = dir.path().join("anibeam-export-full.json");
    let out_path = out.to_string_lossy().into_owned();
    let job = started(core.call(Call::Export { path: out_path.clone(), private: true }).unwrap());
    let done = common::wait_job(&c, job);
    assert_eq!(done.level, Level::Info);
    assert!(matches!(&done.body, EventBody::ExportFinished { path } if *path == out_path), "{done:?}");
    ...
    assert_eq!(doc.series[0].track_choice, Some(TrackChoice { audio: Some(audio), subtitle: Some(SubtitleChoice::Off) }));
    ...
    assert_eq!(prefs.auto_skip, Some(AutoSkip { intro: true, outro: false }));
    assert_eq!(doc.accounts.as_ref().unwrap().anilist.as_ref().unwrap().access_token, "eyJ...");
    ...
    let job = started(core.call(Call::Export { path: library.to_string_lossy().into_owned(), private: false }).unwrap());
    common::wait_job(&c, job);
    let (library, _) = format::parse(&fs::read(&library).unwrap()).unwrap();
    assert!(!library.private);
    assert_eq!(library.series.len(), 3);
    assert!(library.accounts.is_none() && library.keys.is_none() && library.history.is_none() && library.preferences.is_none());
    ...
    assert_eq!(summary.resume_points_merged, 2);
    assert_eq!(summary.accounts_imported, 1);
    assert!(summary.fields_ignored.is_empty(), "{:?}", summary.fields_ignored);

    assert_eq!(
        metadata(&fresh).iter().map(|r| r.series.path.clone()).collect::<Vec<_>>(),
        metadata(&core).iter().map(|r| r.series.path.clone()).collect::<Vec<_>>()
    );
    assert_eq!(views(&fresh), views(&core));
    assert_eq!(completed(&fresh), completed(&core));
    assert_eq!(resume_points(&fresh), resume_points(&core));
    assert!(matches!(fresh.call(Call::GetPreferences).unwrap(), Reply::Preferences { preferences: p } if p == preferences));
    assert!(matches!(fresh.call(Call::GetSettings).unwrap(), Reply::Settings { settings } if settings.auto_skip == AutoSkip { intro: true, outro: false }));
    assert!(matches!(fresh.call(Call::GetTrackers).unwrap(), Reply::Trackers { state } if state.anilist.connected));
}

#[test]
fn a_newer_document_and_an_unreadable_one_fail_at_once() {
    let (dir, core, _c) = common::open_core();
    let newer = write_document(dir.path(), "v3.json", &FIXTURE.replace("\"version\": 1", "\"version\": 3"));
    assert!(matches!(core.call(Call::Import { path: newer }), Err(CoreError::Version { found: 3, supported: 2 })));

    let missing = dir.path().join("not-here.json").to_string_lossy().into_owned();
    assert!(matches!(core.call(Call::Import { path: missing }), Err(CoreError::Invalid { field, .. }) if field == "path"));
}
